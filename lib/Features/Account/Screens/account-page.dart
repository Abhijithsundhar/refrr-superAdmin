...
import 'package:huntrradminweb/Features/Account/Screens/money-page.dart';
...
class AccountTable extends ConsumerStatefulWidget {
  @override
  _AccountTableState createState() => _AccountTableState();
}

class _AccountTableState extends ConsumerState<AccountTable> {
  int currentPage = 0;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  buildListCell('Status', width),
                  buildListCell('Total Credit', width),
                  buildListCell('Total Withdrawal', width),
                  buildListCell('Balance', width),
                  buildListCell('Manage', width),
                ],
              ),
            ),
            SizedBox(height: 8),

            ref.watch(affiliateStreamProvider(searchQuery)).when(
                data: (affiliateList){
                  ...
                  final paginatedData = affiliateList.skip(currentPage * rowsPerPage).take(rowsPerPage).toList();
                    return  ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        int count = data.withdrawalRequest.where((e) => e.status == false).length;
                        String statusText;
                        Color statusColor;
                        if (data.status == 1) {
                          statusText = 'Active';
                        ...
                        return Container(
                          ...
                          child: Row(
                            children: [
                              ...
                              SizedBox(width: width * 0.02),
                              buildListCell(statusText, width, color: statusColor),
                              buildListCell('AED ${data.totalCredit}', width),
                              buildListCell('AED ${data.totalWithrew}', width),
                              buildListCell('AED ${data.totalBalance}', width),
                              buildListCell(InkWell(
                                onTap:() {
                              Future.delayed(Duration(milliseconds : 50),(){
                              showDialog(
                              context: context,
                              // barrierDismissible: false,
                              builder: (_) =>  AddCashDetails(affiliate: data,),
                              );
                              });
                                },
                                  child: Icon(Icons.account_balance_wallet_rounded)), width),
                            ],
                          ),
                        );
                      },
      ...
      bottomNavigationBar: ref.watch(affiliateStreamProvider(searchQuery)).when(
        data: (affiliateList) {
          final totalPages = (affiliateList.length / rowsPerPage).ceil();
          return Padding(
              ...
              children: [
                Row(
                  children: [
                    ...
                    Text(
                      'Showing ${currentPage * rowsPerPage + 1} - ${(currentPage + 1) * rowsPerPage > affiliateList.length ? affiliateList.length : (currentPage + 1) * rowsPerPage} of ${affiliateList.length} entries',
                      style: GoogleFonts.firaSansCondensed(fontSize: width * 0.007, fontWeight: FontWeight.w600),
                    ),
                  ],
                ),
  ...
  Widget buildListCell(dynamic content, double width, {Color? color}) {
    return Expanded(
