...
import 'package:huntrradminweb/Features/Agents/Controllors/agent-controllor.dart';

import '../../../core/constants/color-constants.dart';
import '../../../core/globelvariable.dart';
import '../../home/homepage.dart';
import 'add-agent.dart';
import 'edit-agent.dart';

class AgentsTable extends ConsumerStatefulWidget {
...
class _AgentsTableState extends ConsumerState<AgentsTable> {
  int currentPage = 0;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            ///data row
            ref.watch(agentStreamProvider(searchQuery)).when(
                data: (agentList){
                  ...
                  final paginatedData = agentList.skip(currentPage * rowsPerPage).take(rowsPerPage).toList();
                                  ...
                                  onTap:() {
                                    Future.delayed(Duration(milliseconds : 50),(){
                                      showDialog(
                                        context: context,
                                        // barrierDismissible: false,
                                        builder: (_) =>  EditAgentAlertBox(agent: data ),
                                      );
                                    });
                                  },
                                  child: Center(
      ...
      bottomNavigationBar: ref.watch(affiliateStreamProvider(searchQuery)).when(
        data: (agentList) {
          final totalPages = (agentList.length / rowsPerPage).ceil();
          return Padding(
              ...
              children: [
                Row(
                  children: [
                    ...
                    Text(
                      'Showing ${currentPage * rowsPerPage + 1} - ${(currentPage + 1) * rowsPerPage > agentList.length ? agentList.length : (currentPage + 1) * rowsPerPage} of ${agentList.length} entries',
                      style: GoogleFonts.firaSansCondensed(fontSize: width * 0.007, fontWeight: FontWeight.w600),
                    ),
                  ],
                ),
